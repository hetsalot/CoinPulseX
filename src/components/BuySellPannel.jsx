...
const BuySellPannel = ({ userId, selectedCoin = "BTC", currentPrice, onTradeSuccess }) => {
  ...
  return (
    <div className="glass-card rounded-2xl p-6 shadow-2xl relative overflow-hidden transition-all duration-300">
      ...
      {/* Top Tabs: Buy / Sell */}
      <div className="flex p-1.5 bg-slate-950/70 rounded-xl border border-white/[0.06] mb-6">
        <button
          onClick={() => {
            setAction("buy");
            setError("");
      ...
      <div className="relative mb-4">
        <label className="text-[11px] uppercase font-mono tracking-wider text-slate-400 block mb-2">
          Order Amount
        </label>
        <div className="relative flex items-center">
          <input
            ...
            className="w-full bg-slate-950/70 text-white font-mono text-base sm:text-lg pl-4 pr-20 py-3.5 rounded-xl border border-white/[0.08] focus:outline-none focus:border-cyan-500/50 focus:ring-1 focus:ring-cyan-500/25 transition-all placeholder:text-slate-600"
          />
          <div className="absolute right-3.5 px-2.5 py-1.5 rounded-lg bg-slate-900/90 border border-white/[0.08] text-xs font-bold font-mono text-cyan-400">
            {selectedCoin}
          </div>
        </div>
      </div>
      ...
      <div className="grid grid-cols-4 gap-2 mb-5">
        {[25, 50, 75, 100].map((pct) => (
          <button
            ...
            className="py-2 rounded-xl bg-slate-900/70 hover:bg-slate-800 border border-white/[0.05] hover:border-cyan-500/25 text-slate-300 hover:text-cyan-300 font-mono text-xs font-semibold transition-all cursor-pointer"
          >
            {pct === 100 ? "MAX" : `${pct}%`}
          </button>
        ))}
      ...
      <div className="p-4 rounded-xl bg-slate-950/60 border border-white/[0.05] space-y-2.5 mb-5 text-xs font-mono">
        ...
        <div className="pt-2.5 border-t border-white/[0.06] flex items-center justify-between font-bold">
      ...
      <button
        ...
        className={`w-full py-4 rounded-xl font-bold text-sm tracking-wide transition-all duration-200 cursor-pointer flex items-center justify-center space-x-2 disabled:opacity-40 disabled:cursor-not-allowed shadow-lg ${
          action === "buy"
      ...
      {/* Dynamic Alerts */}
      {error && (
        <div className="mt-4 p-3.5 rounded-xl bg-rose-950/40 border border-rose-500/35 text-rose-300 text-xs flex items-start space-x-2.5 animate-fade-in">
      ...
      {success && (
        <div className="mt-4 p-3.5 rounded-xl bg-emerald-950/40 border border-emerald-500/35 text-emerald-300 text-xs flex items-start space-x-2.5 animate-fade-in">
